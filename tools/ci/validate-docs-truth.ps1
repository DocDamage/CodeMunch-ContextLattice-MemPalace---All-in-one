...
$moduleCount = (Get-ChildItem -Path "module\LLMWorkflow" -Filter "*.ps1" -Recurse |
    Where-Object {
...
$packCount = (Get-ChildItem -Path "packs\manifests" -Filter "*.json" |
    Where-Object {
        Test-Path (Join-Path "packs\registries" ($_.BaseName + ".sources.json"))
    }).Count

$parserCount = (Get-ChildItem -Path "module\LLMWorkflow\extraction" -Filter "*.ps1" |
...
if ($progress -notmatch [regex]::Escape($version)) {
    $drift += "PROGRESS.md does not contain VERSION ($version)"
}
else {
    ...
    if ($headerBlock -and ($headerBlock -notmatch "\*\*Current Version:\*\*\s*$version")) {
        $drift += "PROGRESS.md header version does not match VERSION ($version)"
    }
}

if ($progress -notmatch "\*\*PowerShell Modules:\*\*\s*$moduleCount") {
    $drift += "PROGRESS.md module count does not match actual count ($moduleCount)"
}
if ($progress -notmatch "\*\*Domain Packs:\*\*\s*$packCount") {
    $drift += "PROGRESS.md pack count does not match actual count ($packCount)"
}
if ($progress -notmatch "\*\*Extraction Parsers:\*\*\s*$parserCount") {
    $drift += "PROGRESS.md parser count does not match actual count ($parserCount)"
}
